...
model = torchvision.models.detection.maskrcnn_resnet50_fpn(pretrained=True,trainable_backbone_layers=0)  # load an instance segmentation model pre-trained pre-trained on COCO
...
evaluation=True
if evaluation:
    last_checkpoint_path = os.path.join(model_directory, f"best_iou_checkpoint_{file_name}.torch")
    if exists(last_checkpoint_path):
        model.load_state_dict(torch.load(last_checkpoint_path))
    model.eval()
    idx= np.random.permutation(N_images_evaluation)
    map_= []
    iou_=[]
    ...
    df.to_csv(f'evaluation_{file_name}.csv')
    print (np.mean(map_i), np.mean(iou_i))
