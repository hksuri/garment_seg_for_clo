...
model_directory = os.path.join(root_project, file_name)   
...
if evaluation:
    model.eval()
    batch_= np.arange(0, N_images_evaluation)[0:100]
    map_= []
    iou_=[]
    df= pd.DataFrame()
    sample_evaluation= 20 #500
    for i in range(0,len(batch_),sample_evaluation):
